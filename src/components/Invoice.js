                                ...
                                <table className="table table-striped invoice-items-table">
                                    ...
                                    <tbody>
                                        {invoice.items.map((item, index) => {
                                            ...
                                            const itemDiscount = item.discount || (invoice.discountAmount > 0 ? (itemSubtotal / invoice.subtotal) * invoice.discountAmount : 0);
                                            const taxableValue = itemSubtotal - itemDiscount;
                                            const itemGst = item.gstAmount || (invoice.gstAmount > 0 ? (taxableValue / (invoice.subtotal - invoice.discountAmount)) * invoice.gstAmount : 0);
                                            const itemTotal = taxableValue + itemGst;
                                            ...
                                            return (
                                                <tr key={index}>
                                                    ...
                                                    <td>{item.quantity}</td>
                                                    <td>₹{item.unitPrice.toFixed(2)}</td>
                                                    {invoice.discountAmount > 0 && <td>₹{itemDiscount.toFixed(2)}</td>}
                                                    {invoice.gstAmount > 0 && <td>₹{itemGst.toFixed(2)}</td>}
                                                    <td>₹{itemTotal.toFixed(2)}</td>
                                                </tr>
                                            );
                                        })}
