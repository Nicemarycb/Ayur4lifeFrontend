...
import React, { useState, useEffect } from 'react';
import { Form, Button, Alert, Spinner, Badge, Tooltip, OverlayTrigger } from 'react-bootstrap';
import axios from 'axios';
...
const CouponInput = ({ onCouponApplied, onCouponRemoved, appliedCoupon, orderAmount }) => {
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      ...
      if (response.data.valid) {
        ...
        setCouponCode('');
        setError(null);
        // Refresh available coupons after successful application
        fetchAvailableCoupons();
      }
    } catch (err) {
      setError(err.response?.data?.error || 'Failed to apply coupon');
  ...
  const handleRemoveCoupon = () => {
    onCouponRemoved();
    setError(null);
    // Refresh available coupons after removal
    fetchAvailableCoupons();
  };

  const handleSelectCoupon = (coupon) => {
    if (!coupon.isApplicable) {
      setError(`Add ₹${coupon.additionalAmountNeeded} more to use ${coupon.code}`);
      return;
    }
  ...
  // Tooltip for unavailable coupons
  const renderTooltip = (props, coupon) => (
    <Tooltip id={`tooltip-${coupon.code}`} {...props}>
      {coupon.minOrderAmount > 0 
        ? `Minimum order amount: ₹${coupon.minOrderAmount}. Add ₹${coupon.additionalAmountNeeded} more to use this coupon.`
        ...
        <div className="mb-3">
          <div className="d-flex justify-content-between align-items-center mb-2">
            <small className="text-muted">
              Available Coupons: {availableCoupons.filter(c => c.isApplicable).length} applicable
            </small>
            {loadingCoupons && <Spinner animation="border" size="sm" />}
          </div>
          <div className="d-flex flex-wrap gap-2">
            {availableCoupons.map((coupon) => (
              <OverlayTrigger
                key={coupon.id}
                placement="top"
                overlay={(props) => renderTooltip(props, coupon)}
              >
                <Button
                  variant={coupon.isApplicable ? "outline-success" : "outline-secondary"}
                  size="sm"
                  className="text-nowrap coupon-button"
                  onClick={() => handleSelectCoupon(coupon)}
                  disabled={loading || !coupon.isApplicable}
                  style={{ 
                    opacity: coupon.isApplicable ? 1 : 0.7,
                    cursor: coupon.isApplicable ? 'pointer' : 'not-allowed'
                  }}
                >
                  <div className="d-flex flex-column align-items-center">
                    <strong>{coupon.code}</strong>
