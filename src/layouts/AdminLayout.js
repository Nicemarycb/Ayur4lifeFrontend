...
// AdminLayout.js (Updated with sidebar)
...
const AdminLayout = ({ children }) => {
  ...
  const navItems = [
    ...
    { path: '/admin/coupons', icon: faPercent, label: 'Coupons' },
    { path: '/admin/return-policy', icon: faEdit, label: 'Return Policy' },
    { path: '/admin/return-requests', icon: faEye, label: 'Returns' },
    { path: '/admin/cancellations', icon: faTimes, label: 'Cancellations' }
  ];
  ...
  return (
    <div className={`admin-layout ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
      ...
      <div className="admin-main">
        {/* Top Header */}
        <Navbar bg="white" variant="light" className="admin-topbar">
          <Container fluid>
            <Button variant="link" className="sidebar-toggle-mobile" onClick={toggleSidebar}>
              <FontAwesomeIcon icon={faBars} />
            </Button>
            
            <Navbar.Brand className="d-none d-md-block text-success fw-bold fs-5">
              Welcome Ayur4Life Herbals, {user?.firstName || 'Admin'} 
            </Navbar.Brand>

            <Nav className="ms-auto">
              <Dropdown align="end">
                <Dropdown.Toggle variant="light" id="dropdown-admin-user">
